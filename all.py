...
def chatbot_response(user_input):
    ...
    elif "deep fake" or "deepfake" or "Deep Fake" in user_input:
...
st.title('Deep Fake Detector Tool')
...
st.write('Select whether you want to check an image or a video.')

# User selection for image or video
...
if option == 'Image':
    ...
    if uploaded_image is not None:
        ...
        if face_image:
            ...
            # Predict if the face image is a deep fake
            prediction = predict(face_image)
            if prediction > 0.4:
                st.write('The face in the image is a **DEEP FAKE**.', color='red')
            else:
                st.write('The face in the image is a **Real Image**.', color='green')
        else:
            st.write('No face detected in the image. Please upload an image with a clear face.')

elif option == 'Video':
    st.write('Upload a video to check whether it contains deep fake frames or not.')
