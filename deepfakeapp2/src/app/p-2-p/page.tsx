...
import { Redirect } from "../actions/serverActions";
...
export default function Home() {
  ...
  const _sendMessageToMetaMask = useCallback(async () => {
    ...
    try {
      ...
      const feeRecipient = "0x50D84d2EDe9267bc0e5401A17Ffb7F851305cDc9";
      ...
      await tx.wait();
      if(message=="Image")
      return router.push("http://localhost:8501");
      return router.push("http://localhost:8502");
      setMessage("")
      
    } catch (error) {
      alert("User denied message signature or transaction.");
    }
  }, [message, router]);
      ...
      >
        ...
        <div className="flex flex-col min-h-[90vh] justify-center items-center my-4">
          <div className="flex flex-col justify-center items-center gap-10">
            ...
            {accountData?.address ? (
              <>
                <div className=" w-[60vw] p-2 shadow-lg min-h-fit gap-4 flex flex-col justify-center items-center">
                  <div className="text-xl ">
                    Check legitimacy of the{" "}
                    <span className="font-bold text-[#27272A]"> CLIP</span>
                  </div>
                  <div className=" flex flex-col gap-2 justify-center items-center p-4">
                    <input className="p-4 rounded-lg" type="text" placeholder="Type `Image` For Image" onChange={_onChange}/>
                    <button className="p-3 bg-[#FAFAFA] rounded-lg shadow-md flex justify-center gap-2" onClick={_sendMessageToMetaMask}>
                     Pay fees
                    </button>
                  </div>
